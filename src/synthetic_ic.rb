...
def plot_violation_rates(genuine_rows, infected_rows)
  ...
  gp = <<~GP
    ...
    set output "violatioljjn.png"
    set title "Violation Rates"
    set xlabel "Chip index"
    set ylabel "Violation count"
    set key left top
    plot \
  ...
  gp << genuine_rows.map { |r| "#{r[:chip_idx]} #{r[:violations]}" }.join("\n")
  gp << "\ne\n"
  gp << infected_rows.map { |r| "#{r[:chip_idx]} #{r[:violations]}" }.join("\n")
  gp << "\ne\n"
...
infected  = File.join(REPORTS_DIR, "all_paths_withHT2.rpt")
